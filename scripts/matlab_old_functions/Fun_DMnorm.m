function [V]=Fun_DMnorm(m,n,D,MM,p)
...
%                                   =-1 for cost attributes (expenditure) ??????
    ...
    if p==0  %- Normalización por desviación
       D2=sqrt(sum(D.^2));
       V=D./repmat(D2,m,1); 
    end
    ...
    if p==-4   %- Vector norm. method for benefit & inverse for cost atributes
        for j=1:n
            t(j)=norm(D(:,j),2); 
            V(:,j)= D(:,j)./t(j);
        end 
